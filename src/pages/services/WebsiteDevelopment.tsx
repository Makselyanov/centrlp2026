...
export default function WebsiteDevelopment() {
  const faqItems = [
    ...
    { question: "Как вы используете AI при разработке?", answer: "AI помогает быстрее собирать варианты структуры, группировать факты и проверять повторяющиеся задачи. Оффер, цены, доказательства, итоговые тексты и решения по сайту подтверждаются человеком; AI не заменяет фактическую проверку." },
    { question: "Как быстро окупятся вложения в сайт?", answer: "Срок нельзя определить без данных о спросе, источниках трафика, среднем чеке, марже и обработке обращений. До запуска фиксируем измеримые события, а окупаемость считаем по фактическим расходам, квалифицированным заявкам и продажам." },
    { question: "Что если сайт не будет приносить заявки?", answer: "Сначала отделяем отсутствие трафика от потерь на странице и после формы. Проверяем оффер, мобильный путь, события, источники и получение обращения менеджером; затем отдельно согласуем правки. Позиции, количество заявок и продажи не гарантируются без данных и контролируемого трафика." },
    { question: "Нужно ли мне разбираться в программировании?", answer: "Нет. До старта согласуем, какие данные владелец должен менять самостоятельно. Для простых страниц это может быть система управления контентом; для индивидуального проекта порядок изменений и поддержки фиксируется отдельно." },
    { question: "Вы настраиваете рекламу?", answer: "Да, настройку Яндекс Директа можно заказать отдельно. Перед запуском проверяем посадочную, Метрику и контрольную заявку, чтобы рекламный трафик не шёл на неподготовленный путь." },
    { question: "Можно ли платить по этапам?", answer: "Да, поэтапную оплату можно согласовать после фиксации состава работ и календаря. Конкретные доли и даты указываются в предложении или договоре для проекта." },
  ...
  const audience = [
    ...
    { icon: Rocket, title: "Стартапы и новые ниши", text: "Нужно проверить одну гипотезу без тяжёлого продукта. Решение: согласованный MVP-сайт с одним оффером, формой и измеримым действием." },
    { icon: Zap, title: "Действующий бизнес", text: "Посетители есть, но путь обращения непрозрачен. Решение: проверить оффер, мобильный сценарий, события и доставку заявки до менеджера." },
  ];
  ...
  const painPoints = [
    { text: "Стоимость заявки растёт, но непонятно, проблема в трафике, странице или обработке", icon: CreditCard },
    ...
    { text: "Посетители заходят, но редко доходят до звонка, формы или мессенджера", icon: MousePointer },
  ...
  const process = [
    { step: "01", title: "Анализ ниши и задачи", text: "Изучаем выдачу, предложения конкурентов, отзывы и исходные данные бизнеса. AI используем для ускорения обработки, а факты и выводы проверяем вручную." },
    ...
    { step: "04", title: "Клиентский контент", text: "Собираем проверяемые тексты об услуге, цене, процессе, ограничениях и следующем шаге. Факты подтверждает заказчик." },
  ...
  const results = [
    { title: "Измеримый путь заявки", text: "Источник, CTA, форма, событие и получение обращения" },
    { title: "Полная прозрачность", text: "Дашборды с метриками, а не «ощущения»" },
    { title: "Основа для улучшений", text: "Решения принимаются после фактических данных, а не по обещанному коэффициенту" },
  ...
  const whyUs = [
    ...
    { icon: Zap, title: "AI под контролем", text: "Автоматизируем повторяющиеся операции, но проверяем факты, тексты и результат человеком." },
    ...
    { icon: FileText, title: "Проверяемая приёмка", text: "Фиксируем этапы, критерии готовности, передачу доступов и порядок исправления дефектов." },
    { icon: CreditCard, title: "Поэтапная оплата", text: "Состав, даты и платежи согласуются до начала проекта." },
  ];
