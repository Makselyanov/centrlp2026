...
const faqSchemaByRoute = {
  "/services/custom-crm": [
    {
      ...
      answer:
        "Разработка персональной CRM начинается от 180 000 ₽. В стартовый объём обычно входят карта процесса, роли, карточки клиентов и сделок, основные статусы, импорт данных и подключение ключевого источника заявок.",
    },
    {
      question: "Как принять персональную CRM после разработки?",
      answer:
        "До начала работ фиксируют проверяемые сценарии: заявка создаёт карточку без дубля, источник и UTM сохраняются, ответственный назначается, права ролей соблюдаются, обязательные статусы работают, а руководитель видит согласованный отчёт.",
    },
    {
      question: "Что нужно прислать для первого расчёта CRM?",
      answer:
        "Достаточно перечислить источники заявок, пользователей и их роли, основные этапы работы, документы, текущие таблицы или систему и обязательные интеграции.",
    },
  ],
  "/proverka-saita-i-zayavok-za-48-chasov": [
    ...
    {
      question: "Что входит в стоимость от 15 000 ₽?",
      answer:
        "В стартовую стоимость входит проверка одной основной посадочной страницы и пути заявки: мобильный и десктопный сценарий, форма и быстрые контакты, доступные события Метрики, UTM и доставка контрольного обращения. На выходе — карта потерь и 5–7 приоритетных действий. Внедрение правок, настройка рекламы, CRM и дополнительные страницы оцениваются отдельно после проверки.",
    },
    {
  ...
  "/services/web-analytics": [
    {
      question: "Сколько стоит заказать настройку веб-аналитики для сайта?",
      answer:
        "Базовая настройка начинается от 15 000 ₽ и обычно занимает 3–5 рабочих дней. В неё входят проверка счётчика, цели для формы, телефона и мессенджеров, UTM-разметка, контрольная заявка и проверка, что источник обращения сохраняется. Интеграции с CRM, коллтрекинг и сквозная аналитика рассчитываются отдельно после проверки текущего маршрута заявки.",
    },
    {
    ...
    {
      question: "Как принять настройку веб-аналитики?",
      answer:
        "Проверяем тестовую ссылку с UTM и контрольную заявку: успешная отправка считается отдельно от нажатия кнопки и ошибки, параметры источника сохраняются, а обращение доходит до ответственного, CRM или журнала заявок.",
    },
    {
      question: "Какие доступы и данные нужны для настройки?",
      answer:
        "Для расчёта достаточно ссылки на сайт, списка рекламных каналов и описания, куда сейчас приходят заявки. После согласования используем приглашения с минимально необходимыми правами; счётчики, цели и отчёты остаются в аккаунтах заказчика.",
    },
  ],
  "/crm-dlya-biznesa": [
    {
    ...
    {
      question: "Как проверить CRM перед приёмкой?",
      answer:
        "Откройте тестовую ссылку с UTM-метками и отправьте контрольную заявку. В CRM должна появиться одна сделка без дубля, с сохранёнными источником и страницей, назначенным ответственным и понятным следующим шагом.",
    },
  ],
...
const commercialStaticSections = {
  ...
  "/crm-dlya-biznesa": {
    ...
    sections: [
      {
        title: "Как принимается настроенная CRM",
        items: [
          "контрольная заявка с UTM-метками создаёт одну сделку без дубля",
          "в карточке сохраняются источник, кампания, страница и контакт клиента",
          "назначается ответственный и фиксируется следующий шаг обработки",
          "повторная отправка с тем же идентификатором не создаёт вторую сделку",
        ],
      },
    ],
    related: [
      { label: "Стоимость внедрения CRM", href: "/blog/stoimost-vnedreniya-crm-tyumen" },
      { label: "CRM или таблица", href: "/blog/crm-ili-tablica-uchet-zayavok-tyumen" },
  ...
  "/services/marketing-strategy": {
    lead: "Закажите план маркетинга для бизнеса в Тюмени от 35 000 ₽: оффер, каналы, гипотезы, бюджет, приоритеты и контроль заявок. Экспресс-формат готовим за 7–10 рабочих дней.",
    sections: [
      {
      ...
      {
        title: "Как принять готовый план",
        items: [
          "выбраны 1–2 приоритетные услуги и объяснена последовательность запуска",
          "для каналов указаны тестовый бюджет, роль и условие остановки",
          "зафиксированы посадочная, оффер, цена входа и маршрут заявки",
          "есть план действий на 30–90 дней со сроками и точками сверки",
          "определена базовая точка по подтверждённым заявкам для сравнения результата теста",
        ],
      },
      {
        title: "Что прислать для расчёта",
        items: [
          "нишу, город или регион продаж и 1–2 приоритетные услуги",
          "текущие источники заявок и ориентир по бюджету на тест",
          "ссылки на сайт, рекламу или CRM и описание места, где теряются обращения",
        ],
      },
    ],
    related: [
  ...
  "/services/web-analytics": {
    ...
    sections: [
      ...
      {
        title: "Что считаем подтверждённой заявкой",
        items: [
          "сервер сохраняет обращение и возвращает отдельный идентификатор приёма",
          "повторная отправка с тем же идентификатором не создаёт дубль",
          "источник, UTM и страница входа сохраняются вместе с обращением",
          "конверсия подтверждается только после ответа CRM с номером сделки",
        ],
      },
      {
        title: "Что нужно для расчёта и безопасного старта",
        items: [
          "ссылка на сайт, список рекламных каналов и описание текущего маршрута заявки",
          "для первого расчёта не нужны пароли или полный доступ к аккаунтам",
          "после согласования выдаются отдельные приглашения с минимально необходимыми правами",
          "счётчики, цели и отчёты остаются в аккаунтах заказчика",
        ],
      },
    ],
    related: [
  ...
  "/services/custom-crm": {
    ...
    sections: [
      ...
      {
        title: "Как принять персональную CRM",
        items: [
          "контрольная заявка создаёт карточку без дубля и сохраняет источник и UTM",
          "ответственный, роли, обязательные статусы и следующий шаг работают по согласованному сценарию",
          "руководитель видит согласованный отчёт, а критические сценарии проверены на тестовых данных",
        ],
      },
    ],
    related: [
