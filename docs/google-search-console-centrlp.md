...
| GSC API env | В `.env.seo.local` не заполнены `GSC_CLIENT_ID`, `GSC_CLIENT_SECRET`, `GSC_REFRESH_TOKEN`; `GSC_SITE_URL` должен быть `sc-domain:centrlp.ru` |
...
- В поиске: 23 страницы.
- Исключено: 1 страница.
- SQI: 10.
- `/blog` и новая статья про Роскомнадзор отвечают `200` и имеют title/canonical/description/og-разметку.
